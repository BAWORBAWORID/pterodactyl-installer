...
_upgrade_print_header() {
  ...
  echo -e "${CYAN}${top}${RST}"
  echo -e "${CYAN}${mid}${RST}"
  printf '%b' "${CYAN}║${RST}  ${CYAN_B}ALWAYS CODEX PANEL UPGRADE${RST}"
  printf '%.0s ' $(seq 1 $((W-30)))
  echo -e "${CYAN}║${RST}"
  printf '%b' "${CYAN}║${RST}  ${DIM}${LGRAY}info@alwayscodex.my.id${RST}"
  printf '%.0s ' $(seq 1 $((W-24)))
  echo -e "${CYAN}║${RST}"
  echo -e "${CYAN}${mid}${RST}"
  echo -e "${CYAN}${bot}${RST}"
  echo ""
...
show_upgrade_progress() {
  ...
  echo ""
  echo -e "  ${GRAY}───────────────────────────────────────────────────${RST}"
  echo -e "  ${DIM}${GRAY}Powered By Always Codex - alwayscodex.my.id${RST}"
  sleep 1
}
