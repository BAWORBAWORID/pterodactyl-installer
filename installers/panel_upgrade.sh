...
_upgrade_print_header() {
  ...
  printf '%b' "${AMBER}║${RST}  ${AMBER_B}◈  ALWAYSCODEX PANEL UPGRADE${RST}$(printf ' %.0s' $(seq 1 $((W-31))))${AMBER}║${RST}\n"
  printf '%b' "${AMBER}║${RST}  ${DIM}${LGRAY}support@alwayscodex.my.id  ·  www.alwayscodex.my.id${RST}$(printf ' %.0s' $(seq 1 $((W-51))))${AMBER}║${RST}\n"
  echo -e "${AMBER}${mid}${RST}"
...
perform_upgrade() {
  ...
  show_upgrade_progress 20 "Downloading latest AlwaysCodex panel release..."
  rm -rf "$PANEL_DIR/resources/scripts"

  TMP_ZIP="/tmp/panel-$(date +%s).zip"
  curl -sSLo "$TMP_ZIP" "$PANEL_DL_URL"
  unzip -o "$TMP_ZIP" -d "$PANEL_DIR/resources/scripts" >/dev/null 2>&1
  rm -f "$TMP_ZIP"

  # If zip contains a nested directory, move contents up
  if [ "$(ls -A "$PANEL_DIR/resources/scripts" 2>/dev/null)" = "1" ] && [ -d "$PANEL_DIR/resources/scripts"/*/ ]; then
    mv "$PANEL_DIR/resources/scripts"/*/* "$PANEL_DIR/resources/scripts/"
    rmdir "$PANEL_DIR/resources/scripts"/*/ 2>/dev/null
  fi

  chmod -R 755 "$PANEL_DIR/resources/scripts"
  ...
  echo -e "  ${GRAY}─────────────────────────────────────────────────${RST}"
  echo ""
  echo -e "  ${DIM}${GRAY}Need help? → support@alwayscodex.my.id${RST}"
  echo ""
}
