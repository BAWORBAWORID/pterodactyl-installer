...
# AlwaysCodex                                                                        #
# support@alwayscodex.my.id | https://alwayscodex.my.id                              #
#                                                                                    #
# Copyright (C) 2025 - 2026, AlwaysCodex                                             #
#                                                                                    #
# This is a modified fork of the original pterodactyl-installer script,              #
# customized for use with the AlwaysCodex custom panel. This fork is                 #
# independently maintained by AlwaysCodex and is not affiliated with the             #
...
upgrade_summary() {
  print_brake 62
  output "Upgrade: Pterodactyl Panel -> AlwaysCodex Panel"
  output ""
  output "This will:"
  output "  1. Put your panel into maintenance mode"
  output "  2. Download the latest AlwaysCodex panel release"
  output "  3. Run composer install (no dev, optimized)"
  output "  4. Clear view & config cache"
  output "  5. Run database migrations"
  output "  6. Fix folder permissions"
  output "  7. Restart PHP-FPM"
  output "  8. Bring your panel back online"
  output ""
  warning "Your existing .env and database will NOT be touched."
  warning "Make sure you have a database backup before proceeding!"
  print_brake 62
}

run_upgrade() {
  output "Starting upgrade to AlwaysCodex panel..."
...
handle_existing_installation() {
  ...
  echo ""
  output "[1] Upgrade existing panel to AlwaysCodex Panel"
  ...
  case "$EXISTING_CHOICE" in
    1)
      upgrade_summary
      echo -e -n "\n* Confirm upgrade to AlwaysCodex Panel? (y/N): "
    ...
    2)
      ...
      # Falls through to main() fresh install flow
      ;;
    3|"")
      error "Installation aborted."
      exit 1
...
goodbye_upgrade() {
  print_brake 62
  output "AlwaysCodex Panel upgrade completed!"
  output ""
  output "Your panel has been upgraded to the latest AlwaysCodex release."
  output "Your .env configuration and database remain intact."
  output ""
  output "Thank you for using AlwaysCodex!"
  output "support@alwayscodex.my.id | www.alwayscodex.my.id"
  print_brake 62
}
